...
function fight() {
	...
	// Furtits strikes
		...
		if (youHit !== 0) {
			...
			yourDamage += damageThisRound;
			if (yourDamage === 5) {
				document.getElementById("no-log").innerHTML += "<p class='hide' id='game'>You go to strike, but from its abdomen it sprays a noxious fluid into your eyes and does " + damageThisRound + " damage. Your vision now has trails and you've lost your sense of balance.</p>";
			} else {
				document.getElementById("no-log").innerHTML += "<p class='hide' id='game'>You try to strike it in the mouth, but it sinks its fangs into your forearm and injects a healthy dose of venom. You fall to the ground reeling in pain. You are severely hallucinating now. You think you see another creature on the counter. Your vision turns dark and the last thing you sense is the feeling of prickly insect legs on your chest, then your neck, then face.</p>";
				reset(); 
			}
		}
...
function reset() {
	document.getElementById("buttons").innerHTML = '<button type="button" class="button dragButtons startOver" onclick="location.reload()">Reset</button>';
}